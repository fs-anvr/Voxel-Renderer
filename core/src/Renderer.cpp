...
namespace VoxelEngine {
  ...
  void Renderer::Init() {
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(voxelVertex), &voxelVertex, GL_STATIC_DRAW);

    glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, voxelIndexBuffer);
    glBufferData(GL_ELEMENT_ARRAY_BUFFER, sizeof(voxelIndex), &voxelIndex, GL_STATIC_DRAW);
  ...
  void Renderer::Render(const VoxelModel& model) {
    ...
    glDrawElementsInstanced(GL_TRIANGLES, 36, GL_UNSIGNED_INT, static_cast<void*>(0), model.voxels.size());
  }
}  // namespace VoxelEngine